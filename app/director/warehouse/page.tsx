...
type Totals = { cost: number; retail: number; consignment: number };

export default async function WarehousePage() {
  const supabase = await createClient();
  const [{ data: onHandRaw }, { data: cost }, { data: sellers }] = await Promise.all([
    supabase
      .from("v_stock_on_hand")
      .select("holder_type, seller_id, product_id, product_name, category, quantity, retail_value, consignment_value"),
    supabase.from("v_stock_cost").select("holder_type, seller_id, product_id, cost_value"),
    supabase.from("profiles").select("id, first_name, last_name, seller_color, is_active").eq("role", "seller"),
  ]);

  const costMap = new Map(
    (cost ?? []).map((c) => [`${c.holder_type}|${c.seller_id ?? ""}|${c.product_id}`, Number(c.cost_value)]),
  );
  const rows: OnHand[] = (onHandRaw ?? [])
    ...
    .map((r) => ({
      ...
      cost_value: costMap.get(`${r.holder_type}|${r.seller_id ?? ""}|${r.product_id}`) ?? 0,
    }));
  ...
  const sum = (rs: OnHand[]): Totals =>
    rs.reduce(
      (a, r) => ({ cost: a.cost + r.cost_value, retail: a.retail + r.retail_value, consignment: a.consignment + r.consignment_value }),
      { cost: 0, retail: 0, consignment: 0 },
    );

  const activeSellers = (sellers ?? [])
    .filter((s) => s.is_active)
    .map((s) => ({ id: s.id, first_name: s.first_name, last_name: s.last_name }));
  const transferProducts = warehouse
    .filter((r) => r.quantity > 0)
    .map((r) => ({ id: r.product_id, name: r.product_name, wh_qty: r.quantity }));
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Sahibkar anbarı və satıcıların şəxsi anbarları.</p>
        </div>
        <div className="flex gap-2">
          <ExportButton href="/api/export/warehouse" />
          <TransferDialog sellers={activeSellers} products={transferProducts} />
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <ValueCard title="Ümumi məhsul aktivi" t={sum(rows)} highlight />
      </div>
      ...
      {[...bySeller.entries()].map(([sid, rs]) => {
        ...
        return (
          <StockSection
            key={sid}
            title={s ? `${s.first_name} ${s.last_name}` : "Satıcı"}
            color={s?.seller_color ?? undefined}
            rows={rs}
          />
        );
      })}
...
function ValueCard({ title, t, highlight }: { title: string; t: Totals; highlight?: boolean }) {
  return (
    <Card className={highlight ? "border-foreground/30" : ""}>
      ...
      <CardContent className="space-y-1">
        <Row label="Maya" value={manat(t.cost)} />
        <Row label="Pərakəndə" value={manat(t.retail)} />
        <Row label="Konsiqnasiya" value={manat(t.consignment)} />
      </CardContent>
    </Card>
  );
}
      ...
      <div className="rounded-xl border bg-background">
        <Table>
          ...
          <TableBody>
            {rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-sm text-muted-foreground">Boşdur.</TableCell>
              </TableRow>
            )}
            {rows.map((r) => (
              <TableRow key={r.product_id}>
                ...
                <TableCell className="text-right text-muted-foreground">{manat(r.cost_value)}</TableCell>
