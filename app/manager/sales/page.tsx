...
export default async function ManagerSalesPage() {
  ...
  const [{ data: sales }, { data: sellers }] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Bütün satıcıların satışları (maya dəyəri və mənfəət göstərilmir).</p>
      </div>
      <SalesExplorer rows={rows} sellers={sellerOpts} />
    </div>
  );
}
